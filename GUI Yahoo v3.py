# -*- coding: utf-8 -*-
"""
Created on Thu Apr 13 14:26:55 2017
...
class simpleapp_tk(Tkinter.Tk):
    ...
    def initialize(self):
        ...
        #button = Tkinter.Button(self, text=u"See Comps", command=self.OnButtonClick)
        ...
        for item in companyData:
            ...
            self.labelVariable.set('  {:<5}\t{:>8.2f}\t{:>6.2f}\t{:>8.2f}\t\t{:>6.2f}\t{:>6.2f}\t{:>6.2f}\t{:>6.2f}\t{:>16}\t{:>16}'.format(item, companyData[item]['last'], companyData[item]['change'], companyData[item]['pctChange'], companyData[item]['ret1'], companyData[item]['ret7'], companyData[item]['ret30'], companyData[item]['ret90'], str(companyData[item]['vol']), str(companyData[item]['volAvg'])))
            #self.labelVariable.set('%12s\t%12s' %(item,str(companyData[item]['last'])))
            #self.labelVariable.set('{:<8}\t{:>8}\t{:>8}\t{:>8}\t{:>8}\t{:>8}\t{:>8}\t{:>8}\t{:>8}\t{:>8}'.format(item, str(companyData[item]['last']), str(companyData[item]['change']), str(companyData[item]['pctChange']), str(companyData[item]['ret1']), str(companyData[item]['ret7'], str(companyData[item]['ret30']), str(companyData[item]['ret90']), str(companyData[item]['vol']), str(companyData[item]['volAvg'])))
            #self.labelVariable.set(item + '\t' + str(companyData[item]['last']) + '\t' + str(companyData[item]['change']) + '\t  ' + str(companyData[item]['pctChange']) + '\t' + str(companyData[item]['ret1']) + '\t' + str(companyData[item]['ret7']) + '\t' +str(companyData[item]['ret30']) + '\t' + str(companyData[item]['ret90']) + '\t' + str(companyData[item]['vol']) + '\t\t' + str(companyData[item]['volAvg']))        
            button = Tkinter.Button(self, text=item, command=lambda r = item: self.OnButtonClick(r))            
            #button = Tkinter.Button(self, text=item, command=lambda: self.OnButtonClick(temp))
...
def update_item(item):
  yahoo = Share(item)
  t = 0
  maximum_errors_not_reached = True
  while(True):
    try:
  ...
  if item in companyData:
      if companyData[item]['last'] != last:
          companyData[item]['last'] == last
          companyData[item]['change'] = round(last - companyData[item]['prevClose'],2)
          companyData[item]['pctChange'] = round(change/companyData[item]['prevClose'] * 100,2)
      if companyData[item]['vol'] != vol:
          companyData[item]['vol'] == vol
          
  if item in compData:
      if compData[item]['last'] != last:
          compData[item]['last'] == last
          compData[item]['change'] = round(last - compData[item]['prevClose'],2)
          compData[item]['pctChange'] = round(change/companyData[item]['prevClose'] * 100,2)
      if compData[item]['vol'] != vol:
          compData[item]['vol'] == vol  

def updateScreen():
    label = Tkinter.Label(self, textvariable=self.labelVariable, anchor="w", fg="gold", bg="black")
    label.grid(column=0, row=2, columnspan=9, sticky='EW')    
    self.labelVariable.set('  {:<5}\t{:>7}\t{:>6}\t{:>8}\t\t{:>6}\t{:>6}\t{:>6}\t{:>6}\t{:>16}\t{:>16}'.format(item, 'test', 'test', 'test', 'test', 'test', 'test', 'test', 'test','test'))
    ...
    for item in companyData:
      ...
      if item in companyData:
          if companyData[item]['last'] != last:
              ...
              companyData[item]['change'] = round(last - companyData[item]['prevClose'],2)
              companyData[item]['pctChange'] = round(change/companyData[item]['prevClose'] * 100,2)
              companyData[item]['vol'] == locale.format("%d", vol, grouping=True)
...
#              compData[item]['vol'] == vol  
      ...
      if change_exists:
        print('Something changed. Updating values of %s'%item)
        app.labelVariable = Tkinter.StringVar()        
        label = Tkinter.Label(app, textvariable = app.labelVariable, anchor="w", fg="gold", bg="black")
        label.grid(column=0, row=row_index, columnspan = 9, sticky='EW')
        app.labelVariable.set('  {:<5}\t{:>8.2f}\t{:>6.2f}\t{:>8.2f}\t\t{:>6.2f}\t{:>6.2f}\t{:>6.2f}\t{:>6.2f}\t{:>16}\t{:>16}'.format(item, companyData[item]['last'], companyData[item]['change'], companyData[item]['pctChange'], companyData[item]['ret1'], companyData[item]['ret7'], companyData[item]['ret30'], companyData[item]['ret90'], str(companyData[item]['vol']), str(companyData[item]['volAvg'])))
        button = Tkinter.Button(app, text=item, command=lambda r = item: app.OnButtonClick(r))
        button.grid(column=10, row=row_index)
      
    print('Updated')
    app.after(interval*1000, update)
