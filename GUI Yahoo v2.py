# -*- coding: utf-8 -*-
"""
Created on Wed Apr  5 13:22:10 2017
...
class simpleapp_tk(Tkinter.Tk):
    ...
    def initialize(self):
        ...
        for item in companyData:
            ...
            self.labelVariable.set('  {:<5}\t{:>7}\t{:>6}\t{:>8}\t\t{:>6}\t{:>6}\t{:>6}\t{:>6}\t{:>16}\t{:>16}'.format(item, str(companyData[item]['last']), str(companyData[item]['change']), str(companyData[item]['pctChange']), str(companyData[item]['ret1']), str(companyData[item]['ret7']), str(companyData[item]['ret30']), str(companyData[item]['ret90']), str(companyData[item]['vol']), str(companyData[item]['volAvg'])))
    ...
    def OnButtonClick(self, button_id):
        ...
        #self.entry.focus_set()
        #self.entry.selection_range(0, Tkinter.END)
        # develop method to open new window with comps
        #index = self.grid.row
        #print(self.grid)
        #stock = str(self.grid(column = 0))        
        #print(button_id)
        #self.top = Toplevel()
        #self.grid()        
        #self.top.title("Comps for "+button_id)
        #self.top.geometry("800x300+30+30")
        #self.labelVariable = Tkinter.Label(self, text='Test data')        
        
        
        #label = Tkinter.Label(self, textvariable=self.labelVariable, anchor="w", fg="gold", bg="gray")       
        #label.grid(column=0, row=1, columnspan=9, sticky='EW')
...
def update_item(item):
  ...
  if item in companyData:
      if companyData[item]['last'] != last:
          ...
          companyData[item]['change'] = change
          companyData[item]['pctChange'] = round(change/prevClose * 100,2)
  ...
  if item in compData:
      if compData[item]['last'] != last:
          ...
          compData[item]['change'] = change
          compData[item]['pctChange'] = round(change/prevClose * 100,2)
...
### populate infor from companyData, revise above to do so.....
...
# omitting voldDelta for now, I don't think it's useful
    ...
    compData.setdefault(ticker, {'companyName':'', 'ret1':0, 'ret7':0,
